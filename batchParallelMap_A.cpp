...
private:
    ...
    moodycamel::ConcurrentQueue<std::pair<std::chrono::high_resolution_clock::time_point,std::pair<Key, Value>>> timed_insert_queue;
    moodycamel::ConcurrentQueue<std::pair<Key, Value>> insert_queue;
    moodycamel::ConcurrentQueue<std::pair<std::chrono::high_resolution_clock::time_point,Key>> timed_delete_queue;
...
public:
    ...
    void batch_insert(const Key& key, const Value& value) {
        timed_insert_queue.enqueue(std::make_pair(std::chrono::high_resolution_clock::now(),std::make_pair(key, value)));
        //insert_queue.enqueue(std::make_pair(key, value));
        //if(insert_queue.size_approx() >= approx_size_bound)
        if(timed_insert_queue.size_approx() > approx_size_bound) {
            std::lock_guard<std::mutex> guard(insertMutex);
            auto start = std::chrono::high_resolution_clock::now();
            std::pair<std::chrono::high_resolution_clock::time_point,std::pair<Key, Value>>* timed_tmp = new std::pair<std::chrono::high_resolution_clock::time_point,std::pair<Key, Value>>[dequeue_buffer_size];
            size_t timed_count = timed_insert_queue.try_dequeue_bulk(timed_tmp, dequeue_buffer_size);
            std::pair<Key, Value>* keyvals_tmp = new std::pair<Key, Value>[timed_count];
            std::chrono::high_resolution_clock::time_point* times_tmp = new std::chrono::high_resolution_clock::time_point[timed_count];
            for(int i = 0; i < timed_count; i++) {
                times_tmp[i] = timed_tmp[i].first;
                keyvals_tmp[i] = timed_tmp[i].second;
            }
            std::vector<std::pair<Key, Value>> timed_batch(keyvals_tmp, keyvals_tmp + timed_count);
            std::chrono::high_resolution_clock::time_point stop = std::chrono::high_resolution_clock::now();
            map_ = map_.multi_insert(map_, timed_batch);
            std::chrono::high_resolution_clock::time_point batch_complete = std::chrono::high_resolution_clock::now();
            auto timing_duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            auto latency_sum = std::chrono::duration_cast<std::chrono::microseconds>(stop-stop);
            auto latency_max = std::chrono::duration_cast<std::chrono::microseconds>(stop-stop);
            for(int i = 0; i < timed_count; i++) {
                auto operation_duration = std::chrono::duration_cast<std::chrono::microseconds>(batch_complete - times_tmp[i]);
                latency_sum += operation_duration - timing_duration;
                latency_max = max(latency_max, operation_duration - timing_duration);
            }
            // can sort the array and do percentile latency if desired
            std::cout << "Batch of " << timed_count << " inserts with average latency: "<<(latency_sum.count()/timed_count)<< ", max latency: "<< latency_max.count() << " in microseconds, timing duration:" << timing_duration.count() << " in microseconds" <<std::endl;
        }
    }

    void batch_delete(const Key& key) {
        timed_delete_queue.enqueue(std::make_pair(std::chrono::high_resolution_clock::now(),key));
        if(timed_delete_queue.size_approx() > approx_size_bound) {
            std::lock_guard<std::mutex> guard(deleteMutex);
            auto start = std::chrono::high_resolution_clock::now();
            //std::pair<std::chrono::high_resolution_clock::time_point,Key>tmp[dequeue_buffer_size];
            std::pair<std::chrono::high_resolution_clock::time_point,Key>* timed_tmp = new std::pair<std::chrono::high_resolution_clock::time_point,Key>[dequeue_buffer_size];
            Key tmp[dequeue_buffer_size];
            size_t timed_count = timed_delete_queue.try_dequeue_bulk(timed_tmp, dequeue_buffer_size);
            Key* keys_tmp = new Key[timed_count];
            std::chrono::high_resolution_clock::time_point* times_tmp = new std::chrono::high_resolution_clock::time_point[timed_count];
            for(int i = 0; i < timed_count; i++) {
                times_tmp[i] = timed_tmp[i].first;
                keys_tmp[i] = timed_tmp[i].second;
            }
            std::vector<Key> timed_batch(keys_tmp, keys_tmp + timed_count);
            std::chrono::high_resolution_clock::time_point stop = std::chrono::high_resolution_clock::now();
            map_ = map_.multi_delete(map_, timed_batch);
            std::chrono::high_resolution_clock::time_point batch_complete = std::chrono::high_resolution_clock::now();
            auto timing_duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            auto latency_sum = std::chrono::duration_cast<std::chrono::microseconds>(stop-stop);
            auto latency_max = std::chrono::duration_cast<std::chrono::microseconds>(stop-stop);
            ...
            // can sort the array and do percentile latency if desired
            std::cout << "Batch of " << timed_count << " deletes with average latency: "<<(latency_sum/timed_count).count()<< ", max latency: "<< latency_max.count() << " in microseconds" <<std::endl;
        }
    }
    ...
    void test_insert_latency(int n = 1000000, int num_threads = 1) {
        ...
        if (!data_structure.supports_batch_insert) {
            ...
            for (uint64_t i = 0; i < n; ++i) {
                ...
                //printf( "Time taken by insert %d is %f\n", i, wtime);
            }
        ...
        // Calculate the average and max latency
        double sum = 0;
        double max_latency = 0;
        for (int i = 0; i < durations.size(); ++i) {
            sum += durations[i].count();
            max_latency = std::max(max_latency, durations[i].count());
        }
        std::cout << "Time taken by " << demangled_name << " for " << n << " inserts" << std::endl;
        std::cout << "Average latency: " << sum / n << " microseconds" << std::endl;
        std::cout << "Max latency: " << max_latency << " microseconds" << std::endl;
...
int main(int argc, char * argv[]) {
    ...
    // test3.test_batch_insert_latency(1000000, for i in 1000 10000 100000 1000000 10000000; do ./a.out $i > new$i.out; done, max_threads);
    ...
    // test3.test_batch_insert_latency(10000000, stoi(argv[1]), max_threads);
